...
def expected_value(held_dice, num_die_sides, num_free_dice):
    ...
    expected_value = float(scores) / float(len(combinations))
...
def strategy(hand, num_die_sides):
    ...
    for tup in all_possible_hands_set:
        ...
        exp_value_list.append(exp_val)
        exp_val_dict[tup] = exp_val
    max_ev = max(exp_value_list)
    for key, value in exp_val_dict.items():
        if value == max_ev:
            hand = key
